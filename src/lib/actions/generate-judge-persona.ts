...
export async function generatePersonaFromSession(data: {
  ...
  if (existing) throw new Error("A persona has already been generated from this session");
  ...
  if (!demographics?.cvFileUrl) throw new Error("Session has no CV upload");
  ...
  if (!pathMatch) throw new Error("Cannot parse CV storage path from URL");
  ...
  if (downloadError || !fileData) throw new Error("Failed to download CV file");

  const buffer = Buffer.from(await fileData.arrayBuffer());
  const cvText = await extractCvText(buffer, cvFileName);
  if (!cvText.trim()) throw new Error("No text could be extracted from CV");
